...
export const configSchema = z.object({
  ...
  [configKeys.GITHUB_TOKEN]: z.preprocess(trimToUndefined, z.string().trim().min(1)).optional(),
  ...
  [configKeys.LLM_TEMPERATURE]: z.coerce
    .number()
    .min(0, 'must be between 0 and 2')
    .max(2, 'must be between 0 and 2')
    .default(0.2),
  [configKeys.LLM_MAX_OUTPUT_TOKENS]: z.coerce
    .number()
    .int()
    .positive('must be a positive integer')
    .default(900),
  [configKeys.CONTEXT_MAX_TOKENS]: z.coerce
    .number()
    .int()
    .positive('must be a positive integer')
    .default(8192),
  [configKeys.PROMPT_MAX_CHARS]: z.coerce
    .number()
    .int()
    .positive('must be a positive integer')
    .default(8000),
  [configKeys.ANALYZE_MAX_ISSUES]: z.coerce
    .number()
    .int()
    .positive('must be a positive integer')
    .default(200),
  [configKeys.ISSUE_BODY_MAX_CHARS]: z.coerce
    .number()
    .int()
    .positive('must be a positive integer')
    .default(4000),
